...
- **Visit origin**: every lead carries `origem`/`meio`/`campanha`/`conteudo`. Order: `utm_source|utm_medium|utm_campaign|utm_content` from the URL → value kept in `sessionStorage` from the first visit → `document.referrer` domain with `meio: 'referrer'` → `origem: 'direto'`. A lead with no origin is what ruins campaign reports, so there is always a value. UTM text comes from the URL and goes to the database: it is filtered by `UTM_ACEITO` and capped at `UTM_MAX` (120), on the way in **and** on the way out of `sessionStorage`, which the visitor can edit. Telemetry is wrapped so it can never cost a signup.
- **Which post brought them** (`conteudo`, 22/09/2026): `utm_content`, same sanitisation, on **both** sides of the ratio (`visitas` and `leads`) — the other three fields name the campaign and never separate two creatives inside it. Migration in `docs/conteudo.sql` (`add column if not exists` on both tables; nothing about RLS changes, both are already insert-only). No `utm_content` in the URL means `null` and nothing else changes. The column stores the raw value only: there is **no** efficiency calculation in the code or in the database — reading the numbers is his call.
- **Visit counting (the denominator)**: every page load inserts one row in
  `visitas` (`POST /rest/v1/visitas`, same anon key, insert-only policy —
  migrations in `docs/visitas.sql` and `docs/conteudo.sql`, **each applied by
  hand once**). It exists because
  the project counted signups and nothing else: with no denominator there is no
  conversion rate, and no page change can be shown to work. It lives in
  Supabase rather than Vercel Analytics because the page is served by **two**
  hosts (Vercel and the GitHub Pages mirror) and both get real traffic. Each
  page declares its own name in `<body data-pagina="...">` — deriving it from
  the path would split one file into two report rows, since Vercel serves
  `/imperdiveis` and GitHub Pages serves `/alertas-tech/imperdiveis.html`. One
  visit per page per session (`sessionStorage`); a POST that fails leaves no
  mark, so the next load retries. It carries **no personal data** and is fired
  without `await`: telemetry never holds the page and never costs a signup.
